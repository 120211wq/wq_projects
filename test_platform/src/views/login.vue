      ...
      <el-form  :model="user" inline="true" :rules="rules" ref="user" >
        ...
        <table>
          <tr>
            <td>用户名</td>
            <td>
              <el-input v-model="user.username" :rules="rules" placeholder="请输入用户名"></el-input>
            </td>
          </tr>
          <tr>
            <td>密码</td>
            <td>
              <el-input type="password" :rules="rules" v-model="user.password" placeholder="请输入密码" @keydown.enter="doLogin"></el-input>
          ...
          <tr>
            ...
            <td colspan="2">
              ...
              <el-button style="width: 300px" type="primary" @click="doLogin('user')">登录</el-button>
            </td>
          </tr>
        </table>
        </el-form>
...
import { do_login} from '../api/index'
        ...
        if (valid) {
          ...
          do_login(that.user)
              .then(res => {
                ...
                if (res.status === 200){
                  localStorage['token'] = res.data.token.toString(),
